    ...
    <div class="row">
      <div class="col-lg-12 col-md-10 mx-auto">
        ...
        <!--所以靠他們調整版面一下-->
          ...
          <div class="row">
            <div class="col-lg-12">
              <form action="/BackstageProduct/update2/{{$product2->id}}" method="POST" role="form">
                {{ csrf_field() }}
                {{ method_field('PATCH') }}
                ...
                <font face="微軟正黑體">
                  <div class="form-group">
                    <label>產品編號：</label>
                    <input type="string" class="form-control" name="id" placeholder="請輸入產品編號" value="{{$product2->id}}">
                  </div>
                  <div class="form-group">
                    <label>產品名稱：</label>
                    <input type="string" class="form-control" name="goodname" placeholder="請輸入產品名稱" value="{{$product2->goodname}}">
                  </div>

                  <div class="form-group">
                    <label>價錢：</label>
                    <input type="string" class="form-control" name="price" placeholder="請輸入價錢" value="{{$product2->price}}">
                  </div>

                  <div class="form-group">
                    <label>數量：</label>
                    <input type="string" class="form-control" name="quantity" placeholder="請輸入數量" value="{{$product2->quantity}}">
                  </div>

                  <div class="form-group">
                    <label>有效日期：</label>
                    <input type="string" class="form-control" name="expirydate" placeholder="請輸入有效日期" value="{{$product2->expirydate}}">
                  </div>
